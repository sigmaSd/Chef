...
chef.addMany(
  [
    {
      name: "rr",
      download: async ({ latestVersion, signal }) => {
        await $.request(
        ...
        ).signal(signal).showProgress().pipeToPath();
        //rr-5.6.0-Linux-x86_64.tar.gz
        await $`tar -xzf rr-${latestVersion}-Linux-x86_64.tar.gz`.signal(
          signal,
        );
    ...
    },
    {
      ...
      download: async ({ latestVersion, signal }) => {
        ...
        //FIXME
        await $`chmod +x imhex-${latestVersion}-x86_64.AppImage`.signal(signal);
    ...
    },
    {
      ...
      download: async ({ latestVersion, signal }) => {
        ...
        switch (Deno.build.os) {
          case "linux":
            await $`ar x ${archiveName}`.signal(signal);
            await $`tar -xzf data.tar.gz`.signal(signal);
    ...
    },
    {
      ...
      download: async ({ signal }) => {
        ...
        await $`tar -xzf linux-x64.tar.gz`.signal(signal);
    ...
    },
    {
      ...
      download: async ({ latestVersion, signal }) => {
        ...
        await $`tar -xvf scrcpy-linux-x86_64-${latestVersion}.tar.gz`.signal(
          signal,
        );
        return {
          dir: {
    ...
    },
    {
      ...
      download: async ({ latestVersion, signal }) => {
        ...
        await $`tar -xvf ZAP_${latestVersion.slice(1)}_Linux.tar.gz`.signal(
          signal,
        );
        return {
    ...
    },
    {
      ...
      download: async ({ latestVersion, signal }) => {
        ...
        await $`tar -xvf tinymist-x86_64-unknown-linux-gnu.tar.gz`.signal(
          signal,
        );
        return {
    ...
    },
    {
      ...
      download: async ({ latestVersion, signal }) => {
        ...
        await $`chmod +x HttpToolkit-${version}-x64.AppImage`.signal(signal);
    ...
    },
    {
      ...
      download: async ({ latestVersion, signal }) => {
        ...
        await $`tar -xzf codebook-lsp-x86_64-unknown-linux-musl.tar.gz`.signal(
          signal,
        );
    ...
    },
    {
      ...
      download: async ({ latestVersion, signal }) => {
        ...
        await $`tar -xzf texlab-x86_64-linux.tar.gz`.signal(signal);
        return { exe: "texlab" };
      },
    ...
    },
    {
      ...
      download: async ({ latestVersion, signal }) => {
        ...
        await $`tar -xzf ${archiveName}`.signal(signal);
    ...
    },
    {
      ...
      download: async ({ latestVersion, signal }) => {
        ...
        await $`chmod +x Handy_${version}_amd64.AppImage`.signal(signal);
