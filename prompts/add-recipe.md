...
recipe file is at **`~/.cache/chef/chefjsrdefault/chefjsrdefault.ts`**.
...
**Pass 1: Discover.** Download the real release asset and inspect it in a temp
directory:

- Run curl/wget to download the asset the user points to
- Inspect the file: use `tar -tzf` for archives, `file` to detect type, `ls -R`
  to see extracted structure
...
- If extraction creates a containing directory →
  `{ dir: { path: "./dir/", exe: "./binary" } }`
- If the asset itself is the binary (AppImage, statically linked binary) →
  `chmod +x` then `{ exe }`
- If the asset needs special handling (npm, `.deb`, custom install script) →
  handle accordingly

## Tips

- `$` is Chef's cross-platform shell wrapper (from `@david/dax`). Use template
  strings: <code>$`command`</code>
...
- Use `Deno.build.os` / `Deno.build.arch` when the release provides
  platform-specific assets

## Instructions

1. **Read the existing file** to understand its conventions.
2. **Find the real release URL** (GitHub releases, NPM, etc.) and download it to
   inspect.
